...
def cos_sim(emb1, emb2):
    ...
    norm2 = norm2.reshape(r,-1)
    emb1 = emb1/norm1
    emb2 = emb2/norm2
    emb2 = torch.transpose(emb2, 0, 1)
    sim = torch.trace(torch.matmul(emb1, emb2))/r
    return -1.0*sim
...
def load_feature_graph(dataset, config):
...
def load_topology_graph(dataset,config,hphily,nnode,nclass):
    struct_edges = np.genfromtxt(config.structgraph_path, dtype=np.int32)
    sedges = np.array(list(struct_edges), dtype=np.int32).reshape(struct_edges.shape)
    sadj = sp.coo_matrix((np.ones(sedges.shape[0]), (sedges[:, 0], sedges[:, 1])), shape=(config.n, config.n), dtype=np.float32).toarray()
    l = np.loadtxt(config.label_path, dtype = int)
    node_labels = np.array(l)
    label = defaultdict(list)
    for i in range(nnode):
        label[node_labels[i]].append(i)
    nedges = sadj.sum()
    nhetro = 53790
    # for i in range(nnode):
    #     for j in range(nnode):
    #         if sadj[i,j] ==1 and node_labels[i]!=node_labels[j]:
    #             nhetro+=1
    add_node = int((nedges*hphily - nhetro*100)/(2*(100-hphily)))
    if add_node<=0:
      add_node = 0
    print(add_node)
    for i in range(add_node):
        classes = []
        weight = []
        for j in range(nclass):
            classes.append(j)
            weight.append(len(label[j]))
        n1 = random.randint(0,nnode-1)
        lb = int(node_labels[n1])
        classes.remove(lb)
        weight.pop(lb)
        class_choose = random.choices(classes, weights=weight, k=1)[0]
        flag = False
        while ( flag !=True):
            n2 = random.choice(list(label[class_choose]))
            if sadj[n1,n2] !=1:
                sadj[n1,n2] = 1
                sadj[n2,n1] = 1
                flag = True
